...
async function list(taskId) {
  const { rows } = await db.query(
    `SELECT c.id, c.task_id, c.user_id, c.comment, c.mentions, c.edited_at, c.created_at,
...
async function create(taskId, userId, comment, mentions = []) {
  const { rows } = await db.query(
    `INSERT INTO task_comments (task_id, user_id, comment, mentions) VALUES ($1, $2, $3, $4) RETURNING id`,
    [taskId, userId, comment, JSON.stringify(mentions)]
  );
